...
myData <- read.csv("/Path/to/File.csv", headers=TRUE)
    ...
    <p>Alternately, in RStudio's <code>Environment</code> tab, we can click the <code>Import Dataset</code> button and choose a local file or web URL to read our data from.</p>
    
    <p>Whichever option you choose, be sure to use a .txt, .dat, .csv, or similar file: R can't read many proprietary file formats (E.g., Excel's .xslx; SPSS .sav; etc.). Usually, whatever program you use to manage your data will have an option to allow you to save to CSV that you can access through the <code>File -> Save As</code> menu.</p>
...
Type 'license()' or 'licence()' for distribution details.
    ...
    <p>When you're working in the console, all you have to do is type in your command and hit <code>Enter</code>; the code will be automatically evaluated. Alternately, however, you can use a script by clicking <code>File -> New File -> R Script</code>. This allows you to write multiple lines of R code at once and then selectively run them by holding <code>Ctrl/Cmd + Enter</code> (depending on whether you are using a Mac or PC keyboard). By default, this will execute whatever line is currently selected by your pointer; however, you can also highlight multiple lines, hit <code>Ctrl/Cmd + Enter</code>, and evaluate everything highlighted at once.</p>
...
myMatrix <- matrix(vector, nrow=r, ncol=c, byrow=FALSE, 
    ...
    <p>Here, <code>vector</code> is a vector of elements that you would like to divide into rows and columns; <code>nrow</code> and <code>ncol</code> respectively refer to the number of rows and columns in your matrix; <code>byrow</code> takes a TRUE/FALSE boolean and indicates whether the matrix should be filled in by rows (TRUE) or by columns (FALSE). Finally, <code>dimnames</code> is an optional argument that specifies the column and row names.</p>
...
drugCondition <- factor(drugCondition)
    ...
    <ol class="exercises">
      ...
      <pre>
        <code>
download.file(
    "https://raw.githubusercontent.com/faulconbridge/appliedStats/gh-pages/part1/data/ch02Ex01.csv",
...
weight <- c(sample(95:200,10,replace=TRUE))
      ...
      And now combine the three vectors above with <code>myVector</code> to create a data frame called <code>myDataFrame</code>.</li>
      <li>Almost there! Finally, convert the column <code>sex</code> into a factor. (HINT: to reference a single column of a data frame, try myDataFrame$sex.)</li>
    </ol>
    <br />
